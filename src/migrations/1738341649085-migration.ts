...
export class Migration1738341649085 implements MigrationInterface {
  name = "Migration1738341649085";
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`auction\` (\`auction_id\` varchar(36) NOT NULL, \`title\` varchar(255) NOT NULL DEFAULT 'Auction Title', \`description\` text NOT NULL, \`item\` varchar(255) NOT NULL, \`start_datetime\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`end_datetime\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`status\` enum ('DRAFT', 'PENDING', 'PUBLISHED', 'COMPLETED', 'CANCELLED', 'EXPIRED', 'FAILED', 'STARTED', 'DELETED') NOT NULL DEFAULT 'DRAFT', \`current_highest_bid\` decimal(10,2) NOT NULL DEFAULT '0.00', \`reserve_price\` decimal(10,2) NULL, \`bid_increment\` decimal(10,2) NOT NULL DEFAULT '50000.00', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`created_by_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_701b0a4fcb8b739b9380310435\` (\`item\`), PRIMARY KEY (\`auction_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`user_id\` varchar(36) NOT NULL, \`username\` varchar(255) NOT NULL, \`role\` enum ('admin', 'user') NOT NULL DEFAULT 'user', \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`registration_date\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`last_update\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`is_banned\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`user_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`warning\` (\`warning_id\` varchar(36) NOT NULL, \`reason\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`userUserId\` varchar(36) NULL, PRIMARY KEY (\`warning_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX \`IDX_701b0a4fcb8b739b9380310435\` ON \`auction\``,
    );
